...
function saveMachine(m) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const idxTag = headers.indexOf('TAG');
    // v1.7: usar clientKey e compactKey para match robusto
    const sameId     = String(data[i][idxId]    || '').trim() === String(m.id    || '').trim();
    const sameClientSerial = m.serial && m.client &&
      clientKey(String(data[i][idxCli]  || '')) === clientKey(m.client) &&
      compactKey(String(data[i][idxSerie]|| '')) === compactKey(m.serial);
    const sameClientTag = !m.serial && m.tag && m.client &&
      clientKey(String(data[i][idxCli]  || '')) === clientKey(m.client) &&
      compactKey(String(data[i][idxTag]  || '')) === compactKey(m.tag);
    if (sameId || sameClientSerial || sameClientTag) {
      ...
      return { status: 'ok', action: 'updated', matchedBy: sameId ? 'id' : sameClientSerial ? 'client+serial' : 'client+tag' };
    }
  }

  // v1.7: fallback por série única — evita duplicata quando cliente vem com grafia errada
  if (m.serial) {
...
function ensureMachineFromVisit(body, machineId, visitDate, now) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // v1.7: usar clientKey para comparação robusta
    const match =
      (machineId && rowId === String(machineId).trim()) ||
      (serial && rowSer && compactKey(rowSer) === compactKey(serial) &&
        (clientKey(rowCli) === clientKey(clientCanonical) || clientKey(rowCli) === clientKey(client))) ||
      (!serial && tag && rowTag && compactKey(rowTag) === compactKey(tag) &&
        (clientKey(rowCli) === clientKey(clientCanonical) || clientKey(rowCli) === clientKey(client)));

    if (match) {
      sheet.getRange(i + 1, idxId  + 1).setValue(machineId || rowId);
      if (clientCanonical) sheet.getRange(i + 1, idxCli + 1).setValue(clientCanonical);
  ...
  // v1.7: fallback por série única — evita duplicata quando cliente diverge
  if (serial) {
    const serialCompact = compactKey(serial);
    let uniqueMatch = null;
    let matchCount = 0;
    for (let j = 1; j < data.length; j++) {
      const rowAtivo = String(data[j][headers.indexOf('Ativo')] || 'SIM').toUpperCase();
    ...
    if (matchCount === 1 && uniqueMatch !== null) {
      ...
      Logger.log('ensureMachineFromVisit: serial_unique match linha=' + j + ' clienteExistente="' + rowCli + '" recebido="' + client + '"');
      sheet.getRange(j + 1, idxId  + 1).setValue(machineId || rowId);
      // Preservar o cliente já cadastrado (mais confiável que o vindo do campo)
      if (rowCli) sheet.getRange(j + 1, idxCli + 1).setValue(rowCli);
      else if (clientCanonical) sheet.getRange(j + 1, idxCli + 1).setValue(clientCanonical);
      ...
      return { status: 'ok', action: 'updated', matchedBy: 'serial_unique', clientCorrected: true, machineId: machineId || rowId };
    }
  }

  // Fallback legado: buscar por serial normalizado + cliente normalizado
  if (serial) {
    const _normStr = v => String(v || '').trim().toLowerCase()
      .normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9]/g, '');
    const _normSerBk = _normStr(serial);
    const _normCliBk = _normStr(clientCanonical || client);
    for (let j = 1; j < data.length; j++) {
      if (_normStr(String(data[j][idxSer] || '')) === _normSerBk &&
        ...
        const rowId = String(data[j][idxId] || '').trim();
        sheet.getRange(j + 1, idxId  + 1).setValue(machineId || rowId);
        if (clientCanonical) sheet.getRange(j + 1, idxCli + 1).setValue(clientCanonical);
        if (branch)   sheet.getRange(j + 1, idxFil + 1).setValue(branch);
        if (brand)    sheet.getRange(j + 1, idxMar + 1).setValue(brand);
        if (model)    sheet.getRange(j + 1, idxMod + 1).setValue(model);
        if (serial)   sheet.getRange(j + 1, idxSer + 1).setValue(serial);
