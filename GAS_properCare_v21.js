...
const HEADERS = {
  ...
  CAT_TECNICOS: [ 'id','nome','pin_hash','ativo','perfil' ],
  ORDENS_SERVICO: [
...
function doGet(e) {
  ...
  try {
    ...
    switch (action) {
      case 'ping':
        result = { status: 'ok', version: '21', spreadsheet: SS.getName(), ts: new Date().toISOString() };
        break;
      case 'searchMachine':
...
function loginTecnico(body) {
  ...
  if (!pinHash) return { status:'error', error:'PIN inválido' };
  const tecnicos = getSheetData('CAT_TECNICOS');
  const tec = tecnicos.find(function(t){
    return String(t.pin_hash || '').trim() === pinHash && String(t.ativo || 'SIM').trim().toUpperCase() === 'SIM';
  });
  if (!tec) return { status:'error', error:'PIN inválido' };
  const os = getSheetData('ORDENS_SERVICO').filter(function(o){
    if (String(o.status || '').trim().toLowerCase() === 'concluida') return false;
    return parseJsonArray_(o.tecnicos_vinculados).includes(String(tec.id || ''));
  });
  return { status:'ok', tecnico:{ id:tec.id||'', nome:tec.nome||'', perfil:tec.perfil||'' }, os:os };
}
...
function saveOS(body) {
  ...
  const now = new Date().toISOString();
  const allowed = ['status','maquinas_vinculadas','tecnicos_vinculados','descricao','data_prevista','id_visita_resultado','inicio_atendimento','fim_atendimento','pdf_url','drive_folder_id','drive_folder_url','tipo_os'];
  let rowNum = -1;
  if (id && idx.id_os >= 0) {
    for (let r=1; r<data.length; r++) if (String(data[r][idx.id_os] || '').trim() === id) { rowNum = r + 1; break; }
  }
  if (rowNum < 0) {
    id = id || ('OS-' + Date.now());
    const numero = String(body.numero_os || generateOsNumber_()).trim();
    const obj = {
      id_os:id, numero_os:numero, id_cliente:body.id_cliente||'', cliente:body.cliente||'', descricao:body.descricao||'',
      data_abertura:body.data_abertura||now, data_prevista:body.data_prevista||'', status:body.status||'aberta',
      tecnicos_vinculados: Array.isArray(body.tecnicos_vinculados) ? JSON.stringify(body.tecnicos_vinculados) : (body.tecnicos_vinculados||'[]'),
      maquinas_vinculadas: Array.isArray(body.maquinas_vinculadas) ? JSON.stringify(body.maquinas_vinculadas) : (body.maquinas_vinculadas||'[]'),
      id_visita_resultado:body.id_visita_resultado||'', drive_folder_id:body.drive_folder_id||'', drive_folder_url:body.drive_folder_url||'',
      inicio_atendimento:body.inicio_atendimento||'', fim_atendimento:body.fim_atendimento||'', pdf_url:body.pdf_url||'',
      tipo_os: body.tipo_os || 'Preventiva'
    };
    sheet.appendRow(headers.map(function(h){ return obj[h] || ''; }));
...
function saveTecnico(body) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][idIdx]) === String(body.id || '')) {
      ...
      if (body.perfil !== undefined && perfilIdx >= 0) sheet.getRange(i+1, perfilIdx+1).setValue(body.perfil);
      return { status: 'ok', action: 'updated' };
    }
  }
...
function getOS(params) {
  ...
  rows = rows.filter(function(o){ return String(o.Ativo || 'SIM').trim().toUpperCase() !== 'NÃO'; });
  if (params.tecnico) rows = rows.filter(function(o){ return parseJsonArray_(o.tecnicos_vinculados).includes(String(params.tecnico)); });
  if (params.status) rows = rows.filter(function(o){ return String(o.status || '').toLowerCase() === String(params.status).toLowerCase(); });
  if (params.cliente) rows = rows.filter(function(o){ return String(o.cliente || '').toLowerCase().indexOf(String(params.cliente).toLowerCase()) >= 0; });
...
function updateOSDriveRefs_(numeroOs, fields) {
  if (!numeroOs) return;
  const sheet = getOrCreateSheet('ORDENS_SERVICO', HEADERS.ORDENS_SERVICO);
  const data = sheet.getDataRange().getValues(); if (data.length < 2) return;
  const headers = data[0], idxNumero = headers.indexOf('numero_os');
  if (idxNumero < 0) return;
  for (let r=1; r<data.length; r++) if (String(data[r][idxNumero] || '') === String(numeroOs)) {
...
function salvarFotosDrivePGP(body){
  ...
  if (body.attachmentType === 'pdf_os') refs.pdf_url = url;
  updateOSDriveRefs_(body.osNumero || '', refs);
