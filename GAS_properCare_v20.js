...
function doGet(e) {
  ...
  try {
    ...
    switch (action) {
      case 'ping':
        result = { status: 'ok', version: '20', spreadsheet: SS.getName(), ts: new Date().toISOString() };
        break;
      case 'searchMachine':
...
function saveOS(body) {
  ...
  const now = new Date().toISOString();
  const allowed = ['status','maquinas_vinculadas','tecnicos_vinculados','descricao','data_prevista','id_visita_resultado','inicio_atendimento','fim_atendimento','pdf_url','drive_folder_id','drive_folder_url'];
  let rowNum = -1;
  if (id && idx.id_os >= 0) {
